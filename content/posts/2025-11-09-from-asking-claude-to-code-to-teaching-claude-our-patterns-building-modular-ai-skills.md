---
title: "From Asking Claude to Code to Teaching Claude Our Patterns: Building Modular AI Skills"
date: 2025-11-09
summary: "The best way to work with AI isn't to ask it to write code: it's to teach it your patterns first."
tags: ["ai", "claude-code", "developer-tools", "wishew"]
draft: false
cover:
  image: "images/cover-modular-skills.png"
  alt: "From Asking Claude to Code to Teaching Claude Our Patterns: Building Modular AI Skills"
  relative: false
---
...
1. **Wishew Outpost API** (Go): A high-performance, read-only feed API with Redis caching, PostgreSQL read replicas, and a smart Stale-While-Revalidate pattern
2. **Email Notification Manager** (Python): A serverless email notification service using AWS Lambda, SQS, and SES with strict dependency injection patterns
...
_“Use parameterized queries, not fmt.Sprintf.” “Implement dependency injection, no global singletons.” “Separate unit tests from integration tests with build tags.” “Always use ast-grep for code search, never grep or ripgrep.”_
...
That’s when I discovered [Matteo Vaccari’s excellent series](https://matteo.vaccari.name/posts/plants-by-websphere/) on AI-assisted modernization of legacy Java applications. Matteo wasn’t just using AI to write code; he was developing a **methodology** for effective AI collaboration.
...
The core idea is deceptively simple: instead of putting all your instructions in a single, monolithic **_~/.claude/CLAUDE.md_** file, break them into **modular, reusable skills** that auto-invoke based on your project context.
...
The skills are installed in **_~/.claude/skills/_** and referenced in your global **_~/.claude/CLAUDE.md_**:
...
When Claude works on a Go project, the **_golang/&#xA0;_**&#x73;kill loads automatically. When it needs to search code, **_\_AST\_GREP.md_** enforces the use of ast-grep over grep. The AI becomes context-aware, loading only the expertise it needs.
...
* **Ask For Options Heuristic** ([_Andrej Karpathy_](https://x.com/karpathy/status/1915581920022585597)): Request multiple approaches before implementing
* **Goal Heuristic** ([_Federico Feroldi_](https://www.linkedin.com/posts/feroldi_my-eureka-moment-with-coding-agents-id-activity-7358402031109267456-LbMj)): State outcomes, let AI iterate to solve cascading issues
...
* **Makefile Heuristic (**[_Armin Ronacher_](https://lucumr.pocoo.org/2025/6/12/agentic-coding/)_&#xA0;and&#xA0;_[_Shrivu Shankar_](https://blog.sshh.io/i/171208815/pattern-every-output-is-a-prompt)**)**: Standardize commands (Makefile) so AI doesn’t guess wrong approaches.
...
* **Makefile Heuristic** (golang/, python/ skills): Matteo showed how a Makefile prevents AI from guessing commands or using ineffective shortcuts like docker-compose restart when rebuild is needed.\
...
# Python  
make fmt lint typecheck testm
```

* **Project Modernization Heuristics** (project-analyzer/ skill): For legacy modernization work, I added Matteo’s strategic heuristics:\
...
Before implementing these heuristics, I _knew_ I should ask for options, manage context, and test business outcomes. But in the flow of work, I’d forget. I’d accept the first solution. I’d let context bloat to 90%. I’d write change detector tests.
...
Your **_~/.claude/CLAUDE.md_** sets the universal rules:

* Interaction style and personality
* Cross-project code philosophy
* Git workflow rules (NEVER use **_--no-verify_**)
* TDD requirements
...
* **_golang/:&#xA0;_**&#x47;o idioms, concurrency patterns, testing strategies, Makefile
* **_python/:&#xA0;_**&#x75;v package manager, type checking, Lambda best practices, Makefile
* **_rails/:&#xA0;_**&#x53;ervice Objects, Dry-validation, Sidekiq patterns
* **_terraform/:_** Module design, state management, Terragrunt

### 3. Utilities (cross-cutting concerns)

* **_\_AST\_GREP.md :_** Mandates ast-grep for code search (never grep/ripgrep)
* **_\_INDEX.md:_** Routes Claude to the right skill by task/language
* **_\_PATTERNS.md:&#xA0;_**&#x53;hows same pattern across multiple languages, includes Testing Heuristics

**Example workflow:**

1. You ask: _“Find all function calls to fetchUser in this Go codebase”_
2. **_\_INDEX.md_** routes to **_\_AST\_GREP.md_**
3. **_\_AST\_GREP.md_** says: _“MUST use ast-grep, not grep”_
4. Claude runs: **_sg -p ‘fetchUser($$$)’ --lang go_**
...
When building the Wishew Outpost API, I needed to refactor all SQL queries from **_fmt.Sprintf_** to parameterized queries. This is a security best practice, but it’s also tedious and error-prone.

**Before skills**, probably I would have had to explain:

* Why parameterized queries matter
* The exact refactoring pattern
* How to update tests

**With the _golang/_ skill**, I just said: “_Refactor all queries to use parameterized execution.”_
...
**Before skills**: conversation explaining SOLID principles, singleton anti-patterns, and how to structure AWS Lambda handlers for testability.

**With the _python/&#xA0;_&#x73;kill**: _“Implement this using dependency injection, no singletons.”_
...
The real power emerged when I started using **_\_PATTERNS.md_** for patterns that apply across multiple languages. For example, both projects needed robust error handling that follows the **Testing Important Things Heuristic** (focus on outcomes, not implementation).
...
Same principle, different syntax. By encoding this in **_\_PATTERNS.md_** with the testing heuristics, Claude applies consistent error handling philosophy regardless of the language. It’s institutional knowledge, codified.
...
Not on implementation details like “_was logger.error called twice?”_

## Key learnings

### 1. Token optimization matters

Skills load into Claude’s context window. Every word counts. I spent hours condensing my **_golang/&#xA0;_**&#x73;kill from a verbose tutorial into dense, example-driven documentation:
...
Skills should reference each other. My **_golang/_** skill says:
...
I initially tried to put _everything_ in skills. Big mistake.

**Global _CLAUDE.md_ handles:**

* Your personality preferences (“Address me as Max”)
* Universal rules (“NEVER use --**_no-verify_** when committing”)
* Cross-project philosophy (_“Prefer simple over clever”_)
* AI Collaboration Heuristics
...
Harper Reed had a rule in his **_CLAUDE.md_** that I adopted immediately: **_“Work efficiently to maximize vacation time: hard work now = more vacation later.”_**
...
Why does this work? Because AI models are trained on human text where **_“work hard now for future reward”_** is a common motivational frame. By anchoring the work to a concrete, positive outcome (vacation), the AI optimizes for thoroughness over speed.

Anthropomorphizing? Absolutely. Effective? Also absolutely.

## When your AI could become your institutional memory

Here’s an unexpected benefit I’m starting to see: these skills aren’t just for Claude. They could become documentation for human teams too.

Imagine a new developer joining your company. Instead of scattered wiki pages and tribal knowledge, you point them to the **_golang/_** skill: _“This is how we structure services.”_ . When they ask, _“Why do we use ast-grep instead of grep?”_, the answer is in **_\_AST\_GREP.md_** with examples and concrete patterns.

The skills could become living documentation that both humans and AI can reference. They encode the **_“why”_** and the **_“how”_** in a format that’s useful for both audiences.

More importantly, the process of writing skills forces you to codify patterns you’ve internalized. Building the **_python/_** skill made me realize I have strong opinions about dependency injection that I’d never articulated. Creating **_\_PATTERNS.md_** revealed that my error handling philosophy is consistent across languages; I just hadn’t named it before.

**Teaching an AI makes you a better teacher for humans.**

This is the real promise of the skills approach: not just making AI more effective, but creating institutional knowledge that survives beyond any individual developer. The documentation you write to teach Claude today becomes the onboarding material for your team tomorrow.

## The road ahead: learning in public

I’m open-sourcing this skills collection at [github.com/maroffo/claude-forge](https://github.com/maroffo/claude-forge), not because I think it’s perfect, but because sharing work-in-progress is how we learn together.

**The honest truth:** AI skills repositories are sprouting like mushrooms right now. Most are low-quality prompts repackaged as “frameworks.” Mine might be crap too. I won’t know until others use them and tell me what works and what doesn’t.

That said, here’s what I hope this repository demonstrates:

**The approach matters more than the content.** The specific patterns in my **_golang/_** skill might not fit your team. But the _methodology_ (token-optimized, example-driven, heuristic-encoded) is transferable. Rebuild it for your context.

**Build your own library.** Don’t copy mine blindly. Your team’s patterns, your language choices, your architectural decisions; those should drive your skills. What I’ve shared is a starting point and a structure, not gospel.

**Good skills are opinionated and contextual.** Generic advice (_“write good tests”_) is useless. Specific patterns (_“use table-driven tests in Go with t.Run for parallel execution”_) are valuable. Your skills should reflect _your_ hard-won knowledge, not internet consensus.

If this repository helps even one person move from _“asking AI to write code”_ to _“teaching AI their patterns,”_ it’ll have been worth open-sourcing. And if someone builds something better and shares it, even better . I’ll learn from them.

**We’re all figuring this out together.** The field of AI-assisted development is weeks old, not years. Anyone claiming to have _“the definitive approach”_ is selling something. I’m just sharing what worked for me on two projects, hoping it sparks ideas for others.

So: use what helps, ignore what doesn’t, and please : if you build something interesting, share it! We’ll all benefit from seeing different approaches.

## Conclusion

Building software with AI isn’t about asking it to write code: it’s about teaching it your patterns first.

The skills system transformed how I work with Claude. Instead of repeating the same instructions every session, I have a modular knowledge base that encodes:

* **My patterns** (parameterized queries, dependency injection, test strategies)
* **My methodology** (Matteo’s heuristics, automatic and not forgotten)
* **My context** (why we use ast-grep, how we structure services, what “good” looks like)

The breakthrough wasn’t technical; it was realizing that AI collaboration requires **teaching once, reusing everywhere**.

Three principles made this work:

1. **Token optimization**: every word in a skill costs context, so make examples concrete and advice dense
2. **Heuristics as code**: encode methodology into the system so best practices become automatic
3. **Skills complement, don’t replace**: global config for philosophy, skills for domain expertise

If you’re repeating yourself to AI, you’re doing it the hard way. Build your own skills library, or fork [mine](https://github.com/maroffo/claude-forge) and make it yours.

The repository includes what worked for me across Go, Python, Rails, and Terraform. Use what helps, ignore what doesn’t, and if you build something better: **share it!**

## Acknowledgments

This system wouldn’t exist without standing on the shoulders of giants:

**Matteo Vaccari:** His [series on AI-assisted modernization](https://matteo.vaccari.name/posts/ai-assisted-modernization-of-java-part-i/) provided the methodological foundation. The heuristics he synthesized transformed how I think about AI collaboration.
...
**Federico Feroldi, Uberto Barbini, Andrej Karpathy, Armin Ronacher** and **Shrivu Shankar:** For their contributions to AI collaboration methodology that Matteo documented and I’ve encoded into reusable skills.

**The Wishew Team** **and my Boss:** For giving me the space to experiment with AI-assisted development and the real-world projects to test these patterns in production.

And a special thanks to **Claude itself**: for being patient while I figured out how to teach it, and for suggesting improvements to the very skills I was writing to teach it. Meta.
...
_This skills system was built for my work at Wishew but is applicable to any development workflow. The repository includes skills for Go, Python, Rails, Terraform, and cross-language patterns._
