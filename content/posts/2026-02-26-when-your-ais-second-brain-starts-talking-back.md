---
title: "When Your AI's Second Brain Starts Talking Back"
date: 2026-02-26
summary: "Three layers of knowledge integration between Obsidian and Claude Code: from write-only vault to a feedback loop that promotes patterns into skills."
tags: ["ai", "claude-code", "obsidian", "knowledge-management"]
draft: false
cover:
  image: "/blog/images/cover-post-4-second-brain.png"
  alt: "When Your AI's Second Brain Starts Talking Back"
  relative: false
---
...
The fix isn't one feature, it's three layers that build on each other.
...
![Vault folder structure: Projects and Plans are new, Second Brain/Bookmarks/Clippings unchanged](/blog/images/medium-post-4-vault-structure.png)
...
![Artifact flow: where each artifact goes and its local fallback](/blog/images/medium-post-4-table-artifact-flow.png)
...
![Token budget: read strategy by note size](/blog/images/medium-post-4-table-token-budget.png)
...
Invisible in Obsidian's preview mode, but readable by future Claude sessions. A lightweight way to build up context across sessions without cluttering the notes.

### Layer 3: The Feedback Loop

Layers 1 and 2 move knowledge between the vault and Claude sessions. Layer 3 moves knowledge from the vault *into the skills themselves*.
...
![Skill Candidates table example with weak and strong signals](/blog/images/medium-post-4-table-skill-candidates.png)
...
Anthropic published a [Complete Guide to Building Skills for Claude](https://www.anthropic.com/engineering/claude-code-best-practices): 33 pages of best practices for the very thing claude-forge is built on. We audited all 28 skills against it.
...
Clean skills are what make Layer 3 reliable: if skill files are poorly structured, proposed additions land in the wrong place or get lost in noise.
...
Ninety days from a one-off fix to a pattern the system teaches automatically, with a human gate at every step.

## The Token Budget

A reasonable concern: does this add token overhead to every session?

![Token cost per component and when each loads](/blog/images/medium-post-4-table-token-cost.png)
