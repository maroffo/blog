---
title: "The Missing Step: What a Colleague's Hint Taught Me About AI-Driven Planning"
date: 2026-02-19
summary: "Structured requirements, deviation rules, and the art of selective theft: five concepts borrowed from GSD, twenty rejected."
tags: ["ai", "claude-code", "developer-tools", "automation"]
draft: false
cover:
  image: "/blog/images/cover-post-3-missing-step.png"
  alt: "The Missing Step: What a Colleague's Hint Taught Me About AI-Driven Planning"
  relative: false
---
...
![Workflow before: user request to plan mode to explore codebase to draft plan to approve to implement](/blog/images/medium-post-3-workflow-before.png)
...
Before borrowing anything, I spent a few hours reading GSD's source: 50+ markdown files across commands, workflows, agents, templates, and references. Thorough. Built to handle AI inconsistency at scale.

The comparison was instructive:

![GSD vs claude-forge comparison](/blog/images/medium-post-3-table-comparison.png)

Five gaps. GSD's XML task format, model profiles, progress bars, STATE.md living document, project initialization ceremony: all interesting, none solving problems we actually have. We're developers who use plan mode, not vibecoding a product description into existence.

Out of a large system, a handful of architectural patterns are worth adopting. The rest is token tax.
...
![Workflow after: user request to refine requirements to plan to approve to implement](/blog/images/medium-post-3-workflow-after.png)
...
![Deviation rules: R1-R4](/blog/images/medium-post-3-table-deviation.png)
...
GSD's STATE.md is well-designed; it's also a token tax on every invocation. Our session logs and MEMORY.md already cover cross-session knowledge. Adding a file that must be read and updated by every workflow is overhead, and for single-session work (most of what we do) it's pure waste.
...
Before: the orchestrator could plan, implement, review, and score, but it couldn't *ask the right questions* before planning, couldn't *handle surprises* during implementation, couldn't *preserve state* when pausing, couldn't *stop for human judgment* when it mattered, and couldn't *verify the result actually works* from the user's perspective.

The workflow now looks like this:

![Updated orchestrator workflow](/blog/images/medium-post-3-orchestrator-loop.png)
...
None of these are new ideas. They're table stakes in human engineering processes. The insight (the one PHB triggered) was that our AI workflow was missing them.

## The Principle

The compound-engineering-plugin gave us the research agent and solutions directory. Boris Tane gave us the annotation cycle. GSD gave us requirements refinement, deviation rules, and goal-backward UAT. Same pattern each time: read the source, take 10-20%, compress to fit the token budget.
...
PHB said five words. Andrea said three ("what about UAT?"). They turned into 115 lines of markdown and one new skill.
