...
It is not obviously wrong, and that is the problem. The construction I use, `h_i = h1 + i*h2`, comes from Kirsch and Mitzenmacher, and it leans on h1 and h2 being independent enough. Splitting one 64-bit hash into two halves can give you that, and for a well-mixing hash it usually does. The catch is in "usually": whether it holds comes down to that specific hash's avalanche, not a law, and the suggestion never raises it, because it cannot see which hash I use or how well it mixes. Take the bet and the filter still runs. It still returns answers. If the bet is wrong, it just carries a higher false-positive rate than the math promised, quietly, for as long as it lives, and nothing tells you which case you landed in. (I use two independent hashes so the guarantee never rides on that question.)
...
Maybe. But look at what that defense quietly assumes. It assumes I already knew the inputs were 15 to 40 bytes, that allocation was the bottleneck, that independence was load-bearing. I did not know allocation was the bottleneck until I wrote the benchmark and read the alloc count. That is the whole point. The context that would have saved the model is exactly the execution detail the model cannot see, and neither could I until I ran it. You cannot prompt your way to a measurement you have not taken.

And even with a flawless prompt, the best thing that comes back is a better hypothesis. Not a measurement. "It will be faster, probably" and "it will be faster, probably, once you account for allocations" are the same kind of object. Both still need the benchmark. A sharper prompt improves the guess. It does not turn the guess into a fact.
...
If you cannot cheaply check it at all, distrust it by default. That is the Kirsch-Mitzenmacher bucket, and it is the one that bites, because the cost of a wrong answer you cannot see is unbounded.
